...
def parse_WR(game):
    ...
    with open("raw_matchups","a") as sfile:
        for player in teamwin:
            for enemy in teamlose:
                    sfile.write(str(player['championId']) + '>' + str(enemy['championId']) + '\n')
...
for i in range(1, max_depth):
    print("\nAbout to trawl ids: " + str(len(user_ids)))
    for user_id in [user_id for user_id in user_ids if user_id not in used_user_ids]:
        ...
        sys.stdout.flush()
        restart_line()
        print("Number of id's trawled:" + str(id_number), end="")
